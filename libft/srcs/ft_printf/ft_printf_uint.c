...
char		*ft_printf_uint(t_flags *flags, int base, char upper)
{
	...
	if (flags->smallest)
		arg = ft_lltoabase((U char)va_arg(flags->list, int), base);
	else if (flags->small)
		arg = ft_lltoabase((U short)va_arg(flags->list, int), base);
	else if (flags->big)
		arg = ft_lltoabase(va_arg(flags->list, U long), base);
	else if (flags->biggest)
		arg = ft_lltoabase(va_arg(flags->list, U long long), base);
	else
